    ...
    if not recommendations:
        ...
        with col_top1:
            with st.container(border=True):
                ...
                st.caption(f"Adds about {earliest_rec['detour']:.0f} extra minutes of driving to that day "
                           f"({earliest_rec['drive_before']:.0f} min drive there, {earliest_rec['drive_after']:.0f} min to the next stop)")
                for note in earliest_rec["notes"]:
                    st.warning(note, icon="⚠️")
                
                map_url_time = get_google_maps_url(earliest_rec)
                st.link_button("🗺️ Open full route in Google Maps", map_url_time, use_container_width=True)
        ...
        with col_top2:
            with st.container(border=True):
                ...
                st.caption(f"Adds about {best_detour_rec['detour']:.0f} extra minutes of driving to that day "
                           f"({best_detour_rec['drive_before']:.0f} min drive there, {best_detour_rec['drive_after']:.0f} min to the next stop)")
                for note in best_detour_rec["notes"]:
                    st.warning(note, icon="⚠️")
                
                map_url_detour = get_google_maps_url(best_detour_rec)
                st.link_button("🗺️ Open full route in Google Maps", map_url_detour, use_container_width=True)
        ...
        st.markdown("---")
        csv_data = recs_to_csv(sorted_by_time)
